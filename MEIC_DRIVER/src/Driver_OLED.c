...
#define OLED_SCL_H		GPIO_SetBits(OLED_SCL_GPIO, OLED_SCL_PIN)
#define OLED_SCL_L		GPIO_ResetBits(OLED_SCL_GPIO, OLED_SCL_PIN)
#define OLED_SDA_H		GPIO_SetBits(OLED_SDA_GPIO, OLED_SDA_PIN)
#define OLED_SDA_L		GPIO_ResetBits(OLED_SDA_GPIO, OLED_SDA_PIN)
#define OLED_SDA_Read GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_1)
#define OLED_SDA_IN  {GPIOB->MODER&=~(3<<(1*2));GPIOB->MODER|=0<<1*2;}	//PB1输入模式
#define OLED_SDA_OUT {GPIOB->MODER&=~(3<<(1*2));GPIOB->MODER|=1<<1*2;} //PB1输出模式
...
//用于us级延时
void OLED_IIC_Delay(u32 time)
{
	while(time--);
}
#define OLED_DELAY_4 OLED_IIC_Delay(10)  //4US
#define OLED_DELAY_1 OLED_IIC_Delay(10)  //4US//1US
#define OLED_DELAY_2 OLED_IIC_Delay(10)  //4US//2US
...
u8 OLED_Wait_Ack(void)
{
	...
	OLED_SDA_H;OLED_DELAY_1;	   
	OLED_SCL_H;OLED_DELAY_1;	 
	while(OLED_SDA_Read)
	{
		ucErrTime++;
		if(ucErrTime>250)
...
{                        
    ...
    {              
			if ((txd&0x80)>>7)
				...
				OLED_SDA_L;
      txd<<=1; 	  
		  OLED_DELAY_2;   //对TEA5767这三个延时都是必须的
		  OLED_SCL_H;
		  OLED_DELAY_2; 
		  OLED_SCL_L;	
		  OLED_DELAY_2;
...
//1 失败
//0 成功
	u8 OLED_WriteData(u8 dev_addr,u8 reg_addr,u8 data)
{
	OLED_Start();
	OLED_Write_Byte(dev_addr);
	if (OLED_Wait_Ack())
		return 1;
  OLED_Write_Byte(reg_addr);
 	if (OLED_Wait_Ack())
		return 1;
...
u8 OLED_Write_Data(uint8_t OLED_data);
u8 OLED_Write_Command(uint8_t OLED_command);
...
u8 OLED_Write_Data(uint8_t OLED_data)
{
	u8 sign = 0;
//	OLED_Start();
	sign = OLED_WriteData(OLED_ADDR,0x40,OLED_data);
	//OLED_Stop();
	return sign;
}
...
u8 OLED_Write_Command(uint8_t OLED_command)
{
	u8 sign = 0;
	OLED_Start();
	sign = OLED_WriteData(OLED_ADDR,0x00,OLED_command);
	OLED_Stop();
  return sign;
}
...
{ 
	OLED_Write_Command(0xb0+page);        //设置page，范围b0~b7
	OLED_Write_Command(((x&0xf0)>>4)|0x10);//set the upper column address

	OLED_Write_Command(x&0x0f);//set the lower column address
}
  ...
  if(y<-8||y>=OLED_HEIGHT||x<0||x>=OLED_WIDTH)
    ...
    if(y<0){
      ...
      if(isInOnePage){
        ...
        if(isRefresh==IS){
          OLED_SetPos(x,y);
          OLED_Write_Data(OLED_Gram[x][page1]);
        }
      }else{
        //显存不改变的相反取位
        bitsChose = 0xff<<(y%8);
        ...
        if(isRefresh==IS){
          ...
          if(page2<OLED_PAGE){
            OLED_SetPos(x,y+8);
            OLED_Write_Data(OLED_Gram[x][page2]);
          }
        }
      }
    }
...
void OLED_Init(void)
{
  ...
  GPIO_Init(GPIOB, &GPIO_InitStructure);
	...
	while(1)
	{
	sign = OLED_Write_Command(0xae);//关闭显示--turn off oled panel	
	printf("%d\r\n",sign);
	}
	...
	OLED_Write_Command(OLED_CMD_Brightness); //--Set SEG Output Current Brightness
	...
	OLED_Write_Command(0xa4);//--Entire Display(off:0xa4/on:0xa5)
	OLED_Write_Command(0xa6);//--Inverse Display(off:0xa6/on:0xa7) 
